      //Main citation: these code main inspired by internet link below, some functions simulate these visualisations,main from the first link(Shawn Allen's visualization) and some other important citation also included in this code
      //http://prag.ma/code/d3-cartogram/ and http://andrewerrity.com/d3-project/ and https://limn.co.za/2013/10/making-a-cartogram/#:~:text=d3.cartogram%20uses%20d3.topojson%20to%20create%20the%20features%20data,like%20this%3A%20var%20features%20%3D%20carto.features%20%28topology%2C%20geometries%29
      //https://schuelerzahlen-zuerich.opendata.iwi.unibe.ch/App2/d3geo-2/ch_var/index.html#pop_female/1991 and http://dev.centrogeo.org.mx/viz_desaparecidos/lib/d3-cartogram/?segmentized#intlmig/2011 and https://schuelerzahlen-zuerich.opendata.iwi.unibe.ch/App2/d3geo-2/
      ...
      var layer = map.append("g").attr("id", "layer"), wards_p = layer.append("g").attr("id", "wards_p").selectAll("path"); // Setting up the data binding layer
      var topology,geometries,rawData,dataById = {}, carto = d3.cartogram().projection(proj).properties(function(d) {return dataById[d.id];}).value(function(d) {return +d.properties[field];});
      ...
      d3.json(url, function(topo) { //Reading HC area actual business data
        topology = topo;
        geometries = topology.objects.wards_p.geometries;
      ...
      function init() {
        var features = carto.features(topology, geometries),  path = d3.geo.path().projection(proj); // map built
        wards_p = wards_p.data(features).enter().append("path").attr("class", "county") //give it path and color
      ...
      function refresh() {
        ...
        wards_p.data(features).transition().duration(700).attr("fill", function (e) { //Colouring the map with the area category after refreshing it
                return colour_map[e.properties.Category]
            })
            .attr("d", path);
        ...
        wards_p.select("title").text(function(d) {return d.properties.NAME;});
      ...
      function updateAndDrawHcMap() {
          ...
          if (field.id === "none") {refresh();}
              ...
              var key = field.key.replace("%d", ""),
                  ...
                  values = wards_p.data().map(value).filter(function (n) {return !isNaN(n);}).sort(d3.ascending),
                  LCI = values[0],
                  HCI = values[values.length - 1];
              var scale = d3.scale.linear().domain([LCI, HCI]).range([1, 100]);
              carto.value(function (d) {return scale(value(d));});
              var features = carto(topology, geometries).features;
              wards_p.data(features).select("title").text(function (d) {
